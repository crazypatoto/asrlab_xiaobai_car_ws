...
#include <AnhungControl/change_map.h>
...
#include "tf/transform_broadcaster.h"//new add
...
public:
	...
	void change_map_callback(const AnhungControl::change_map& msg);//new add
...
private:
	...
	ros::Subscriber cartoSubscriber_;
	...
	int max_map_height;
	float m_resolution;
	void getorigin(std::string file_buf, Eigen::Vector3f &map_origin);
	Eigen::Vector3f map_origin;
	...
	std::string yamlPATH;
	std::string change_map_name;//new add
