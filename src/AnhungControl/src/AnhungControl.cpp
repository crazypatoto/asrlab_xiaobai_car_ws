...
Control::Control(char *address, char *SRV_IP, int port)
{
  ...
  SetLocalpar(LocalparPATH);
	...
	CarName = "NULL";
	Carnumber = int2str(-1);
	MAP_NAME = Lc_par.map_name;
	change_map_name = MAP_NAME;
	...
	imagePATH = "/home/user/Downloads/" + MAP_NAME + ".pgm";
	yamlPATH = "/home/user/Downloads/" + MAP_NAME + ".yaml";
	char *imagePATH_buf = const_cast<char *>(imagePATH.c_str());
	getorigin(yamlPATH, map_origin);

	// QImage map_image;
	// map_image = QImage(imagePATH_buf);

	cv::Mat map_image = cv::imread(imagePATH_buf, CV_LOAD_IMAGE_COLOR);
	max_map_width = map_image.cols;
	max_map_height = map_image.rows;
	// max_map_width = map_image.width();
	// max_map_height = map_image.height();
	std::cout<<"max_map_width = "<<max_map_width<<std::endl;
	std::cout<<"max_map_height = "<<max_map_height<<std::endl;
	...
	if(isSocketOpen_){
		...
		Command_Publisher_ = nodeHandle_.advertise<AnhungControl::change_map>("Command",10);
		...
		cartoSubscriber_ = nodeHandle_.subscribe("pose_nav", 5, &Control::cartoCallback, this);
		...
		change_map_sub = nodeHandle_.subscribe("change_map", 1000, &Control::change_map_callback, this);//new add
		poseSubscriber_ = nodeHandle_.subscribe("laser_odom", 1000, &Control::poseCallback, this);//new add
...
void Control::getorigin(std::string file_buf, Eigen::Vector3f &map_origin)
{
	  std::fstream fin;
		...
		else
		{
				...
				origin.erase(std::remove(origin.begin(), origin.end(), ' '), origin.end());
				std::stringstream cut(origin);
				while(getline(cut,cut_par,','))
				{
						origin_pose[count]=cut_par;
						count++;
				}
				map_origin.x() = ::atof(origin_pose[0].c_str());
				map_origin.y() = ::atof(origin_pose[1].c_str());
				map_origin.z() = ::atof(origin_pose[2].c_str());
		}
}
...
void Control::SetLocalpar(std::string file_buf)
{
    ...
    if(!fin.is_open())
    {
        ...
        std::getline(fin, network_card_temp);
				...
				std::getline(fin, mapname_temp);
        std::getline(fin, mapname_temp);
				Lc_par.map_name = mapname_temp;
    }
...
void Control::RevProcess(double receive_period)
{
	...
	std::string mapping_string = "";//odom use
		...
		if(n == -1){
						...
						if(count>0&& count<end)
						{
							...
							switch (count1){
								...
								default:
									...
									else if(recv_pkg_subitem[1]=="15")
									{
											item>>node_buf.mapping_pose;
											std::cout<<"mapping_string = "<<node_buf.mapping_pose<<std::endl;
									}
									else {
			...
			if(isMisson_real)
			{
				for(int i=0;i<NodeSet.size();i++)
				{
					...
					msg.map=NodeSet[i].map;
					msg.floor=NodeSet[i].floor;
					msg.mapping_pose = NodeSet[i].mapping_pose;
...
void Control::change_map_callback(const AnhungControl::change_map& msg)//new add
{
		if(msg.Name!="")
		{
				change_map_name = msg.Name;
		}
}
...
void Control::cartoCallback(const geometry_msgs::Pose2D& msg)
{
	static int count =0;
	count+=1;

	robot_pose_ = Eigen::Vector3f(msg.x, msg.y, msg.theta);
	//ROS_INFO("Robot pose x: %f y: %f yaw: %f", robot_pose_[0], robot_pose_[1], robot_pose_[2]);
	...
	if(MAP_NAME != change_map_name)//new add
	{
			MAP_NAME = change_map_name;
			imagePATH = "/home/user/Downloads/" + MAP_NAME + ".pgm";
			yamlPATH = "/home/user/Downloads/" + MAP_NAME + ".yaml";
			char *imagePATH_buf = const_cast<char *>(imagePATH.c_str());
			getorigin(yamlPATH, map_origin);
	...
	robot_pose_p.x() = fabs(robot_pose_.x() - map_origin.x()) / m_resolution;
	robot_pose_p.y() = max_map_height - fabs(robot_pose_.y() - map_origin.y()) / m_resolution;
	robot_pose_p.z() = msg.theta / M_PI * 180 * 10;
	...
	if(count==20)
	{
		...
		std::string DataFlow = head + MAP_NAME + ","
		+robot_pose_x + "," + robot_pose_y + "," + robot_pose_z  + ","
		+ real_pose_x + "," + real_pose_y + "," + real_pose_z + ","
...
void Control::poseCallback(const nav_msgs::Odometry& msg)
{
	//std::cout<<"test000"<<isReveice_joystick<<std::endl;
	...
	robot_pose_ = Eigen::Vector3f(test.getOrigin().x(), test.getOrigin().y(), tf::getYaw(test.getRotation()));
	//ROS_INFO("Robot pose x: %f y: %f yaw: %f", robot_pose_[0], robot_pose_[1], robot_pose_[2]);
	...
	robot_pose_p.x() = fabs(robot_pose_.x() - map_origin.x()) / m_resolution;
	robot_pose_p.y() = max_map_height - fabs(robot_pose_.y() - map_origin.y()) / m_resolution;
	robot_pose_p.z() = tf::getYaw(test.getRotation()) / M_PI * 180 * 10;
	...
	//std::cout<<"test"<<isReveice_joystick<<std::endl;
	if(isReveice_joystick){
