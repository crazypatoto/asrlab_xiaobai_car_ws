...
diff_vw::diff_vw(char* dev_name, int Baudrate):Move_Robot(dev_name, Baudrate)
{
  ...
  _pose_init_sub = node_.subscribe("initialpose", 1000, &diff_vw::init_pose_callback, this);//new add
	change_map_sub = node_.subscribe("change_map", 1000, &diff_vw::change_map_callback, this);//new add
...
void diff_vw::init_pose_callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr &msg)//new add
{
...
void diff_vw::change_map_callback(const move_robot::change_map& msg)//new add
{
...
bool diff_vw::Tracking_Angle_Init(int &subpath_index, bool isReSet)
{
	...
	if(transfer_odom)
	{
			...
			slam_pose_.y() = transfer_odom_pose.y();
...
//std::cout<<"AA"<<std::endl;
	...
	if(now_index < First_tracking ){
        ...
        if(Endangle)
        {
            ...
            std::cout<<"=========Endangle============="<<std::endl;
						if(back_trajectory)
						{
								if(target_pos.z()>0)
								{
										target_pos.z() = -1 * (M_PI - target_pos.z());
								}
								else
								{
										target_pos.z() = M_PI + target_pos.z();
								}
						}
		...
		{
				sendreceive.Package_Diff_encoder(-1*cmd_velocity,W_rw,
						command
						);
				//sendreceive.Package_publicWheel_encoder(-1*cmd_velocity, 0, W_rw, 0, command);
		}
...
void diff_vw::Calculate_odom()
{
	...
	if(transfer_odom)
	{
			...
			tra.robot2world(real_odom, origin_localization, real_x, real_y);
