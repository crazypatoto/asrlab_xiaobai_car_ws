...
class DWA_test
{
		public:
			...
			controlU DynamicWindowApproach(state cx, Eigen::Vector3f goal, float * evalParam, std::vector<Eigen::Vector3f> ob, std::vector<Eigen::Vector3f> robot_ob, float R, float goal_distance, int** oscillation_cost, std::vector<Eigen::Vector3f> &predict_pose, std::vector<Eigen::Vector3f> &choose_pose, std::vector<Eigen::Vector3f> &obs_pose);
			state CarState(state cx, controlU u);
			state GenerateTrajectory(state cx, std::vector<state> *traj, float vt, float wt, float predict_dis, float goal_distance);
			state robot_GenerateTrajectory(state cx, float vt, float wt, float predict_dis, float goal_distance);
			maxmotion CalcDynamicWindow(state cx, float *model);
			float CalcHeadingEval(state cx, Eigen::Vector3f goal);
			float CalcDistEval(state cx, std::vector<Eigen::Vector3f> ob, float R, int &idx);
			...
			void Evaluation(state cx, std::vector<eval_db> *EvalDb, Eigen::Vector3f goal, std::vector<Eigen::Vector3f> ob, std::vector<Eigen::Vector3f> robot_ob, float R, float predict_dis, float goal_distance, int** oscillation_cost, std::vector<Eigen::Vector3f> &obs_pose);
...
controlU DWA_test::DynamicWindowApproach(state cx, Eigen::Vector3f goal, float * evalParam, std::vector<Eigen::Vector3f> ob, std::vector<Eigen::Vector3f> robot_ob,float R, float goal_distance, int** oscillation_cost, std::vector<Eigen::Vector3f> &predict_pose, std::vector<Eigen::Vector3f> &choose_pose, std::vector<Eigen::Vector3f> &obs_pose)
{
		controlU u;
		std::vector<eval_db> EvalDb;
		//评价函数的计算 evalDB N*5  每行一组可用参数 分别为 速度、角速度、航向得分、距离得分、速度得分
		Evaluation(cx, &EvalDb, goal, ob, robot_ob, R, evalParam[6], goal_distance, oscillation_cost, obs_pose);  //predict_dis = evalParam(4) 评价函数参数[heading, dist, velocity, predictDistance]
		if (EvalDb.empty())
		{
		...
		for (int i = 0; i < EvalDb.size(); ++i)
		{
				...
				predict_pose.push_back(pose_buf);
		}
		...
		Eigen::Vector3f choose_pose_buf(EvalDb[idx].pose.x, EvalDb[idx].pose.y, 0.0);
		choose_pose.push_back(choose_pose_buf);
...
void DWA_test::Evaluation(state cx, std::vector<eval_db> *EvalDb, Eigen::Vector3f goal, std::vector<Eigen::Vector3f> ob, std::vector<Eigen::Vector3f> robot_ob, float R, float predict_dis, float goal_distance, int** oscillation_cost, std::vector<Eigen::Vector3f> &obs_pose)
{
    EvalDb->clear();
    std::vector<state> traj;
    for (float vt = 0.4; vt < 0.5; vt = vt + 0.1)//根据速度分辨率遍历所有可用速度： 最小速度和最大速度 之间 速度分辨率 递增
    {
      	for (float wt = -0.8; wt <= 0.8; wt = wt + 0.1)//根据角度分辨率遍历所有可用角速度： 最小角速度和最大角速度 之间 角度分辨率 递增
        {
        		//轨迹推测; 得到 xt : 机器人向前运动后的预测位姿; traj: 当前时刻 到 预测时刻之间的轨迹（由轨迹点组成）
        		state xt = GenerateTrajectory(cx, &traj, vt, wt, predict_dis, goal_distance); //predict_dis = evalParam(4), 前向模拟距
						state robot_xt = robot_GenerateTrajectory(cx, vt, wt, predict_dis, goal_distance);
						//各评价函数的计算
        		float heading = CalcHeadingEval(xt, goal);//前项预测终点的航向得分  偏差越小分数越高 w1
						int obs_idx = 0;
        		float dist = CalcDistEval(xt, ob, R, obs_idx);//前项预测终点 距离最近障碍物的间隙得分 距离越远分数越高 w2
        		...
        		//float vel = fabs(vt) + fabs(wt_);//速度得分 速度越快分越高 w3
						...
						if(pixel_x >= 0 && pixel_x <= map_width && pixel_y >= 0 && pixel_y <= map_height)
						{
								...
								oscillation = (1 - oscillation_cost[pixel_x][pixel_y]) + (1 - (tra_num / (float)(20 * 20 * M_PI)));
						}
        		...
        		//float stopDist = CalcBreakingDist(vel, model[MD_ACC]); //制动距离的计算
						...
						//std::cout<<"dist = "<<dist<<" stopDist = "<<stopDist<<std::endl;
						if(!arrival_goal)
						{
								if (dist > 1.4 * R)
								{
										EvalDb->push_back(db);
										Eigen::Vector3f obs_pose_buf(ob[obs_idx].x(), ob[obs_idx].y(), 0.0);
										obs_pose.push_back(obs_pose_buf);
								}
						}
...
void DWA_test::NormalizeEval(std::vector<eval_db> *EvalDb)
{
    //评价函数正则化
...
state DWA_test::robot_GenerateTrajectory(state cx, float vt, float wt, float predict_dis, float goal_distance)
{
	float time = 0.0;
	controlU u = { vt, wt };
...
state DWA_test::GenerateTrajectory(state cx, std::vector<state> *traj, float vt, float wt, float predict_dis, float goal_distance)
{
    float time = 0.0;
    controlU u = { vt, wt };
    traj->clear();
...
float DWA_test::DegreeToRadian(float degree)
{
    return degree / 180 * pi;
}

float DWA_test::RadianToDegree(float radian)
{
    return radian / pi * 180;
}

state DWA_test::CarState(state cx, controlU u)
{
    state result;
...
maxmotion DWA_test::CalcDynamicWindow(state cx, float *model)
{
    maxmotion V_r;
...
float DWA_test::CalcHeadingEval(state cx, Eigen::Vector3f goal)
{
...
float DWA_test::CalcDistEval(state cx, std::vector<Eigen::Vector3f> ob, float R, int &idx)
{
    float dist = 100.0;
...
float DWA_test::Calcgoal_distance(state cx, Eigen::Vector3f goal)
{
...
int DWA_test::Calculate_Uobs(state cx, std::vector<Eigen::Vector3f> ob)
{
...
int DWA_test::history_number(int pixel_x, int pixel_y, int range_R, int** oscillation_cost)
{
		int count = 0;
...
void DWA_test::get_mapinfo(int width, int height, float x, float y, float resolution)
{
		map_width = width;
		map_height = height;
		origin_x = x;
...
float DWA_test::CalcBreakingDist(float vel, float mdacc)
{
    float stopDist = 0;
...
void DWA_test::robot2world(Eigen::Vector3f target_pos, Eigen::Vector3f robot_pos, float &x, float &y)
{
...
void DWA_test::world2robot(Eigen::Vector3f target_pos, Eigen::Vector3f robot_pos, float &x, float &y)
{
