...
#include <move_robot/cartographer_finish.h>//new add
#include <move_robot/test.h>//new add
#include <move_robot/start.h>//new add
#include <move_robot/change_map.h>//new add
#include "trans.h"
#include "kinematics.h"
#include "sendrev.h"
#include "geometry_msgs/PoseWithCovarianceStamped.h"//new add
#include <geometry_msgs/Pose2D.h>//new add
...
#define P_STATE_ODOM 11//new add odom
#define P_STATE_SET_POSE 12//new add odom
...
#define MISSON_ODOM 14
#define MISSON_SET_POSE 15
...
public:
  ...
  int calc_target_index_key(Eigen::Vector3f &robot_pose, float &dis, std::vector<Eigen::Vector3f> &subpath, int &now_index);
	int calc_back_index(Eigen::Vector3f &robot_pose, float &robot_v, std::vector<Eigen::Vector3f> &subpath, int &now_index);
	//joystick
	//void joystick_move();
	...
	void cartoCallback(const geometry_msgs::Pose2D& msg);
	...
	void pathplanCallback_(const nav_msgs::Path& msg);
	...
	void carto_location(geometry_msgs::Pose initial_pose);
...
protected:
	...
	ros::Publisher navigation_Publisher_;
	...
	ros::Subscriber poseSubscriber_;//new add
//	ros::Subscriber Clear_ObsModeSubscriber_;
	...
	ros::Subscriber _pose_init_sub;//new add
	ros::Subscriber cartoSubscriber_;//new add
	ros::Subscriber change_map_sub;//new add
	ros::Subscriber result_classify_Sub;//new add
	ros::Publisher Command_Publisher_;//new add
	//cartographer
  int traj_id;
  ...
  tf::TransformBroadcaster odom_broadcaster;
	...
	bool back_trajectory;

	tf::TransformListener tf_buffer_;

	//test odom to map
	Eigen::Vector3f origin_localization;
	Eigen::Vector3f origin_odom;
	Eigen::Vector3f transfer_odom_pose;
	bool transfer_odom;
	float Rev_V, Rev_W, Rev_a, Rev_x, Rev_y, Rev_th;
...
Move_Robot::Move_Robot(char *dev_name, int Baudrate)
{
    ...
    floor = -1;
		...
		///////////calculate odom
		origin_localization = Eigen::Vector3f(0.0, 0.0, 0.0);
		origin_odom = Eigen::Vector3f(0.0, 0.0, 0.0);
		transfer_odom_pose = Eigen::Vector3f(0.0, 0.0, 0.0);
		transfer_odom = false;
		Rev_V = 0.0;
		Rev_W = 0.0;
		Rev_a = 0.0;
    ...
    //control_timer = node_.createTimer(ros::Duration(time_sample), &Move_Robot::timerCallback, this);
		current_time = ros::Time::now(); //odom test
	  last_time = ros::Time::now(); //odom test
}
...
void Move_Robot::carto_location(geometry_msgs::Pose initial_pose)
{
		ros::ServiceClient client_traj_finish = node_.serviceClient<move_robot::test>("finish_trajectory");
		move_robot::test srv_traj_finish;
		...
		ros::ServiceClient client_traj_start = node_.serviceClient<move_robot::start>("start_trajectory");
		move_robot::start srv_traj_start;
		srv_traj_start.request.configuration_directory = "/home/user/cartographer_ws/install_isolated/share/cartographer_ros/configuration_files";//.lua文件所在路径
		srv_traj_start.request.configuration_basename = "hokuyo_2d.lua";//lua文件
		srv_traj_start.request.use_initial_pose = 1;
		srv_traj_start.request.initial_pose = initial_pose;
...
// }
void Move_Robot::pathplanCallback_(const nav_msgs::Path& msg)
{
		std::cout<<"pathplanCallback_"<<std::endl;
		// std::vector<Eigen::Vector3f> drawpoint;
		// for(int i=0;i<msg.poses.size();i++)
		// {
		// 		Eigen::Vector3f drawpoint_buf;
		// 		tf::Pose pose_tf;
		// 		tf::poseMsgToTF(msg.poses[i].pose, pose_tf);
		// 		drawpoint_buf.x() = msg.poses[i].pose.position.x;
		// 		drawpoint_buf.y() = msg.poses[i].pose.position.y;
		// 		drawpoint_buf.z() =	tf::getYaw(pose_tf.getRotation());
		// 		drawpoint.push_back(drawpoint_buf);
		// }
		// draw(5 , 1.0, 1.0, 0.0, drawpoint);
		///////////////////////////////////////////做路線
		NODE_recv node_buf;
    node_buf.id = 0;/////start
    node_buf.type = 0;
    node_buf.kin = "diff";
    node_buf.time = 0;
    node_buf.btn_finish = true;
    node_buf.node_pose.x() = msg.poses[0].pose.position.x;
    node_buf.node_pose.y() = msg.poses[0].pose.position.y;
		tf::Pose pose_tf;
		tf::poseMsgToTF(msg.poses[0].pose, pose_tf);
    node_buf.node_pose.z() = tf::getYaw(pose_tf.getRotation());
    node_buf.line = 0;
    node_buf.radius = 0.0;
		node_buf.map = "test";
    node_buf.floor = 0;
		node_buf.mapping_pose = "test";
    ...
    node_buf.time = 2;
    node_buf.btn_finish = true;
    node_buf.node_pose.x() = msg.poses[msg.poses.size()-1].pose.position.x;
    node_buf.node_pose.y() = msg.poses[msg.poses.size()-1].pose.position.y;
		tf::poseMsgToTF(msg.poses[msg.poses.size()-1].pose, pose_tf);
    node_buf.node_pose.z() = tf::getYaw(pose_tf.getRotation());
    node_buf.line = 0;
    node_buf.radius = 0.0;
		node_buf.map = "test";
		...
		for(int i=0;i<Traffic_Node.size();i++)
    {
		...
		sub_missonPath_subPoint_buf.start = Traffic_Node[0].id;
		sub_missonPath_subPoint_buf.start_type = Traffic_Node[0].type;
		sub_missonPath_subPoint_buf.end = Traffic_Node[1].id;
		sub_missonPath_subPoint_buf.end_type = Traffic_Node[1].type;
		sub_missonPath_subPoint_buf.map = "test";
		sub_missonPath_subPoint_buf.floor = Traffic_Node[1].floor;
		for(int i=0;i<msg.poses.size();i++)
		{
				Eigen::Vector3f drawpoint_buf;
				tf::Pose pose_tf;
				...
				drawpoint_buf.y() = msg.poses[i].pose.position.y;
				drawpoint_buf.z() =	tf::getYaw(pose_tf.getRotation());
				sub_missonPath_subPoint_buf.sub_missonPath_subPoint.push_back(drawpoint_buf);
		}
		//draw(5 , 1.0, 1.0, 0.0, sub_missonPath_subPoint_buf.sub_missonPath_subPoint);
		sub_missonPath_buf.sub_missonPath.push_back(sub_missonPath_subPoint_buf);
		A_misson.push_back(sub_missonPath_buf);
		if(A_misson.size() == 1)p_state_ = P_STATE_MOVE;
		Traffic_Node.clear();
		///////////////////////////////////////////做路線
}
...
void Move_Robot::IdTypeCallback(const move_robot::Node_recv& command)
{
    ...
    node_buf.floor=command.floor;
		node_buf.mapping_pose = command.mapping_pose;
...
void Move_Robot::Misson_state()
{
    ...
    static bool send_changemap = false;
		static bool send_initial_pose = false;
		static bool odom_reset = true;
		transfer_odom = false;//new add
		float th = 0.0;//new add
    ...
    switch(type){
        ...
        break;
            ...
            Command_STATE = Command_Elevator_Inside;
						static int send_change_count = 0;//send change map
            ...
            ini_pose =  A_misson[now_A_misson].sub_missonPath[now_path_index + 1].sub_missonPath_subPoint[0];
						if(!send_changemap)
						{
								...
								if(send_change_count < 5)
								{
										if(send_change_count == 0)
										{
										...
										{
												std::cout<<"wait change map" <<std::endl;
										}
										send_change_count++;
								}
								else
            ...
            // {
						...
						// 	msg.Name = Change_Map_name;
						// 	// Command_Publisher_.publish(msg);
            //   //   std::string command_str ="Load Map";
            //   //   move_robot::change_map msg;
			...
			if(changemap_finish)
			{
				  std::cout<<"********************changemap_finish"<<std::endl;
					if(!send_initial_pose)
					{
							...
							initial_pose.position.x = ini_pose.x();
							initial_pose.position.y = ini_pose.y();
							initial_pose.position.z = 0.0;
							geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromYaw(ini_pose.z());
	...
	//std::cout<<"ini_pose  " <<ini_pose<<std::endl;
		...
		case MISSON_ODOM:
		std::cout<<"MISSON_ODOM" <<std::endl;
		static int wait_reset_cnt = 0;
		if(odom_reset)
		{
		...
		if(wait_reset_cnt < 10)
		{
				wait_reset_cnt++;
		}
		else
		{
			std::cout<<"wait odom reset" <<std::endl;
			////////////////////////////////////////計算轉換
			th = Rev_th / 180.0 * M_PI;
			...
			odom_reset = true;
			////////////////////////////////////////計算轉換
			p_state_ = P_STATE_ODOM;
			wait_reset_cnt = 0;
		}
		break;

		case MISSON_SET_POSE:
		std::cout<<"MISSON_SET_POSE" <<std::endl;
		float mapping_x = 0.0;
		float mapping_y = 0.0;
		float mapping_z = 0.0;
		for(int i=0;i<A_misson[now_A_misson].ALL_pathnode.size();i++)
		{
				if(A_misson[now_A_misson].ALL_pathnode[i].id==id)
				{
					  int mapping_count = 0;
					  std::string cut_par;
						std::string mapping_pose_buf = A_misson[now_A_misson].ALL_pathnode[i].mapping_pose;
						std::stringstream cut(mapping_pose_buf);
						while(getline(cut,cut_par,'/'))
						{
								if(mapping_count == 0)mapping_x = std::atof(cut_par.c_str());
								else if(mapping_count == 1)mapping_y = std::atof(cut_par.c_str());
								else if(mapping_count == 2)mapping_z = std::atof(cut_par.c_str());
								mapping_count++;
						}
						break;
				}
		}
		std::cout<<"mapping_x = "<<mapping_x<<" mapping_y = "<<mapping_y<<" mapping_z = "<<mapping_z<<std::endl;
		geometry_msgs::Pose initial_pose;
		initial_pose.position.x = mapping_x;
		initial_pose.position.y = mapping_y;
		initial_pose.position.z = 0.0;
		geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromYaw(mapping_z);
		initial_pose.orientation = odom_quat;
		carto_location(initial_pose);
		p_state_ = P_STATE_SET_POSE;
		break;
...
int Move_Robot::calc_back_index(Eigen::Vector3f &robot_pose, float &robot_v, std::vector<Eigen::Vector3f> &subpath, int &now_index)
{
		float min_error_value = 10000000;
		int min_index = -1;
		//當前路徑所有點與車子計算距離取最小
...
void Move_Robot::poseCallback(const nav_msgs::Odometry& msg)
{
		tf::StampedTransform test;
		try
		{
				tf_buffer_.lookupTransform("map", "base_link", ros::Time(0), test);
		}
		...
		if(!back_trajectory)
		{
				slam_pose_ = Eigen::Vector3f(test.getOrigin().x(), test.getOrigin().y(), tf::getYaw(test.getRotation()));
		}
		else
		{
				float back_theta = 0.0;
				if(tf::getYaw(test.getRotation()) > 0)
				{
						back_theta = -1 * (M_PI - tf::getYaw(test.getRotation()));
				}
				else
				{
						back_theta = M_PI + tf::getYaw(test.getRotation());
				}
				slam_pose_ = Eigen::Vector3f(test.getOrigin().x(), test.getOrigin().y(), back_theta);
		}
		if(!isReveice_joystick)
		{
				...
				msg1.id = Command_id;
				msg1.state = Command_STATE;
				//std::cout<<"Command_STATE = "<<Command_STATE<<std::endl;
				STATE_Publisher_.publish(msg1);
		}
}

void Move_Robot::cartoCallback(const geometry_msgs::Pose2D& msg)
{
		if(!back_trajectory)
		{
				slam_pose_ = Eigen::Vector3f(msg.x, msg.y, msg.theta);
		}
		else
		{
			  float back_theta = 0.0;
				if(msg.theta > 0)
				{
						back_theta = -1 * (M_PI - msg.theta);
				}
				else
				{
						back_theta = M_PI + msg.theta;
				}
				slam_pose_ = Eigen::Vector3f(msg.x, msg.y, back_theta);
		}
}
